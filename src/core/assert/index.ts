...
export class Assert {
  static fail(actual, expected, message?) {
    throw new AssertionError(
      message || `Failed value: ${util.inspect(actual)}; ${expected !== undefined ? `Expect: ${util.inspect(expected.name || expected)}` : ''}`,
    );
  }
  ...
  static array(value, { required = false, notEmpty = false, message = '', of = [] } = {}) {
    ...
    if (value && value.length && of.length && !value.every((i) => of.includes(i.constructor))) {
  ...
  static integer(value, { required = false, positive = false, message = '' } = {}) {
    ...
    if (value !== undefined && Number.isInteger(value) && positive && value < 0) {
